...
# The output array/list will also contain only single digit at every index.
# Note:
...
def sum_of_lists(size1, arr1, size2, arr2):
    index = max(size1, size2)
    print(index)
    i = size1 - 1
    j = size2 - 1
    output = (index + 1) * [0]
    ...
    carry = 0
    while i >= 0 and j >= 0:
        output[index] = (arr1[i] + arr2[j] + carry) % 10
        carry = (arr1[i] + arr2[j] + carry) // 10
        i -= 1
        j -= 1
        index -= 1
    while i >= 0:
        output[index] = (arr1[i] + carry) % 10
        carry = (arr1[i] + carry) // 10
        index -= 1
        i -= 1
    while j >= 0:
        output[index] = (arr2[j] + carry) % 10
...
output = sum_of_lists(size1, arr1, size2, arr2)
print(*(output))